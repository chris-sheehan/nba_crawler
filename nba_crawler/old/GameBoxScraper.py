...
# _URL_PATTERN_GAME_BOX = "http://www.sports-reference.com/cbb/boxscores/{game_id}.html"
...
class GameBoxScraper(object):
	...
	def get_stat_tbls(self):
		self.stat_tbls = self.soup.findAll('table', {'class':'sortable  stats_table'})
	...
	def get_player_stats(self, tbl, flds):
		...
		for player in player_rows:
			stats = {fld : cell.text for fld, cell in zip(flds, player.findAll('td'))}
			player_id = self.get_player_id(player.find('td'))
			stats['player_id'] = player_id
			team = tbl.attrs['id'].replace('_advanced', '')
			stats['team'] = team
